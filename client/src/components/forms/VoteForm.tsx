...
const VoteForm: React.FC<Props> = ({ instruction, user, product }) => {
  ...
  useEffect(() => {
    setLike(likes.some(p => p === instruction.id))
  ...
  const handleLike: React.MouseEventHandler<HTMLElement> = (event) => {
    ...
    userService.editLike(instruction.id)
      .then((response) => {
        setVotes(response.resource.instruction.score)
        setLikes(response.resource.likes)
        setDislikes(response.resource.dislikes)
        ...
        product.instructions = product.instructions.sort((a, b) => b.score - a.score)
    ...
    <div>
      {user !== null ? (
        <Container id='vote-element' >
          <Row>
            <Container id={'votes' + instruction.id}>
              {votes}
              <ButtonGroup vertical className='better-votes'>
                <OverlayTrigger placement="top" overlay=
                  {like ? <Tooltip id={'likeButtonTooltip' + instruction.id}>Haluan perua tykkäykseni.</Tooltip>
                    : <Tooltip id={'likeButtonTooltip' + instruction.id}>Mielestäni tämä on suositeltava kierrätysohje.</Tooltip>}>
                  <Button variant={like ? 'success' : 'outline-success'} id={'likeButton' + instruction.id} onClick={handleLike} >
                ...
                <OverlayTrigger placement="top" overlay=
                  {disLike ? <Tooltip id={'dislikeButtonTooltip' + instruction.id}>Haluan perua ei-tykkäykseni.</Tooltip>
                    : <Tooltip id={'dislikeButtonTooltip' + instruction.id}>Mielestäni tämä kierrätysohje ei ole suositeltava.</Tooltip>}>
                  <Button variant={disLike ? 'danger' : 'outline-danger'} id={'dislikeButton' + instruction.id} onClick={handleDislike}>
