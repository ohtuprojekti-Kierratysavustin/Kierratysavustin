...
const VoteForm: React.FC<Props> = ( { instruction, user, product }  ) => {
  ...
  const handleLike: React.MouseEventHandler<HTMLElement> = (event) => {
    ...
    if(like){
      ...
      setLike(true)
      userService.addLike(instruction.id).then(i => setLikes(likes.concat(instruction.id))).then(i => setVotes(instruction.score))
      instruction.score += 1
      if(disLike){
        instruction.score += 1
  ...
  const handleDislike: React.MouseEventHandler<HTMLElement> = (event) => {
    ...
    if(disLike){
      ...
      userService.addDislike(instruction.id).then(i => setDislikes(dislikes.concat(instruction.id))).then(i => setVotes(instruction.score))
    }
